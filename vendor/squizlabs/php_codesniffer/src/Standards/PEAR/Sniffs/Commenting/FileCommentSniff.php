...
class FileCommentSniff implements Sniff
{
    ...
    /**
     * Process the property_category tag.
     *
     * @param \PHP_CodeSniffer\Files\File $phpcsFile The file being scanned.
     ...
     * @return void
     */
    protected function processproperty_category($phpcsFile, array $tags)
    {
        $tokens = $phpcsFile->getTokens();
        foreach ($tags as $tag) {
            ...
            if (Common::isUnderscoreName($content) !== true) {
                ...
                $error     = 'property_category name "%s" is not valid; consider "%s" instead';
                $validName = trim($newName, '_');
                $data      = [
                    $content,
                    $validName,
                ];
                $phpcsFile->addError($error, $tag, 'Invalidproperty_category', $data);
            }
        } //end foreach

    } //end processproperty_category()
